...
jQuery(document).ready(function($) {
	...
	(function() {
		...
		if($('#map').length) {
			$('#map').gMap({ 
				address: 'Erith Leisure Centre, Avenue Road, Erith DA8 3AT, London, United Kingdom',
				zoom: 14,
				markers: [
					{'address' : 'Erith Leisure Centre, Avenue Road, Erith DA8 3AT, United Kingdom'}
				]
			});  
		}
				...
				$.post("php/contact-send.php", data, function(response) {
					...
					if(response.is_errors){
						...
						$.each(response.info,function(input_name, input_label) {
							...
							$response.find('p').append('Alert - Required Field - '+input_label+'!!'+ '</br>');
						});
						...
						if(response.info == 'server_fail'){
							$response.find('p').append('Server failed. Try again later!');
						}
					}
